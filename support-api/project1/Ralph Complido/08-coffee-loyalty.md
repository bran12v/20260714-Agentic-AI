...
- **Enrichment in the write path:** When a record is created (and re-run when the fields it depends on change on update), call a deployed Azure OpenAI chat model to produce structured enrichment and persist it on the record. For this theme: on customer/order activity, generate and store a drink recommendation (name, size, one-sentence rationale) derived from past order history.
